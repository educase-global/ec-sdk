...
const Utils = module.exports = { // Public functions accessible by other modules
  ...
  *********************************************************************/
  ...
  @param {string}  - Search field to be validated

  @return {boolean} - Returns true if size of string is greater than 2 else returns false 
  *********************************************************************/

  searchStringAuthentication : function(text){

    if(Utils.isString(text) && text.length>2)
    return true;
    return false; 
  ...
  *********************************************************************/
  ...
  convertToHumanReadableDate: function(string){

     return moment(string).format("dddd, MMMM Do YYYY, h:mm:ss a")
