...
const getErrorMessagesFromStatusCode = (response) => {
	switch (response.status) {
		case 400:
			return response && response.data || "(Bad Request) The request is not understood by the server due to an incorrect syntax."
			break;
		case 401:
			return response && response.data || "(Unauthorized) It means that one or both of your username and password were invalid."
			break;
		case 403:
		...
		case 404:
			...
			break;
		case 409:
			return response && response.data || "The request could not be completed due to a conflict with the current state of the resource."
			break;
		case 500:
...
const _HttpNodeJS = {
  ...
  *********************************************************************/
	fetch: function (
		...
		// Normalize method: SDK passes lowercase ("put", "post"); branch logic must match both cases
		...
		if (methodNorm === "post" || methodNorm === "put" || methodNorm === "patch" || methodNorm === "delete") {
		...
		instance["http_connection"]
			...
			.catch(function (error) {
				...
				if (error.response) {
					...
					// }
					return cb(
						getErrorMessagesFromStatusCode(error.response), // No error
						error.response.status,
