...
function serveStaleAndRevalidate(cb, cacheKey, url, http_method, http_params, auth_bearer_token, noCache) {
    ...
    if (http_method.toUpperCase() === 'GET' && !noCache) {
        ...
        revalidateCache(cacheKey, url, http_method, http_params, auth_bearer_token, servedFromCache ? null : cb, noCache);
    } else {
        // For non-GET requests, just make the API call
        revalidateCache(cacheKey, url, http_method, http_params, auth_bearer_token, cb, noCache);
    }
}

// Revalidate cache by making the API call
function revalidateCache(cacheKey, url, http_method, http_params, auth_bearer_token, cb, noCache) {
    Lib.HttpHandler.fetchJSON(
        function (err, http_status, http_response_headers, response_data) {
            ...
            if (!Lib.Utils.isNullOrUndefined(response_data)) {
                if (http_method.toUpperCase() === 'GET' && http_status === 200 && !noCache) {
                    const cacheEntry = {
                        data: response_data,
                        status: http_status,
                        timestamp: Date.now(),
...
const Commons = {
    /********************************************************************
      ...
      *********************************************************************/
    httpHandler: function (cb, api_urn, http_method, http_params, auth_bearer_token, noCache) {
        // console.log(noCache, http_params)
