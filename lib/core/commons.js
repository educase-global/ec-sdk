...
function loadCacheFromLocalStorage() {
    ...
    // // If the user has changed or the refresh flag is true, clear the cache
    if (refreshFlag === '1') {
        ...
        // Load cache if user and refresh flag are the same
...
function getCurrentUser() {
    // Parse the user data stored in localStorage
...
const Commons = {
    /********************************************************************
      ...
      *********************************************************************/
    httpHandler: function (cb, api_urn, http_method, http_params, auth_bearer_token) {
        ...
        if (http_method.toUpperCase() === 'GET') {
            if (refreshFlag === '2' && inMemoryCache.has(cacheKey)) {
                    ...
                    try {
                        ...
                        if (parsedCache[cacheKey]) {
                            ...
                            if (!isExpired) {
                                ...
                                return cb(null, cachedResponse.status, null, cachedResponse.data);
                            } else {
        ...
        Lib.HttpHandler.fetchJSON(
            function (err, http_status, http_response_headers, response_data) {
                // Error caused by connectivity or server status
                if (err) {
